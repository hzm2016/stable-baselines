...
class Buffer_option(object):
    def __init__(self, state_dim, action_dim, capacity):
        ...
        self.buffer = np.zeros(shape=(capacity, state_dim * 2 + action_dim + 1 + 1 + 1))
        self.isFilled = False
        self.pointer = 0

    def store(self, state, option, action, reward, next_state, terminal):
        """Store the transition"""

        # flatten the transition tuple
        transition = np.hstack((state, option, action, reward, next_state, terminal))
